...
def main_application_sesmg():
    ...
    with udu_page:
        tab_upload, tab_runtinme = st.tabs(["Uploading", "Runtime optimization"])
        with tab_upload:
            st.file_uploader(label="Upload your model definition sheet.", help="muss noch geschrieben werden.")
        with tab_runtinme:
            with st.expander("Time series preparation"):
                ...
                with col4:
                    st.selectbox("Cluster Criterion", ["None", "Hut", "Hose"])
                with col5:
                    st.selectbox("Season", ["None", "Hut", "Hose"])

            with st.expander("Pre-modeling"):
                st.write("pre model")
    ...
    with st.sidebar.form("Input Parameters"):
        ...
        input_num_threads = st.slider("Number of threads", min_value=1, max_value=35,
                                      help="Number of threads to use on your machine")
        # Choosing solver
        ...
        with st.expander("Pre-Modeling Settings"):
            ...
            input_premodeling_timeseries_criterion = st.selectbox("Cluster Criterion (Pre-Model)",
                                                                  timeseries_cluster_criteria_list)
    ...
    if submitted_optimization:
        ...
        if scenario_input_sheet_path != "":
            ...
            st.write("path input", scenario_input_sheet_path)
            ...
            res_folder_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'results')
            ...
            if not input_activate_premodeling:
                sesmg_main(
