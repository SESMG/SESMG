...
**Attention**: The current version of the US-Tool does create model definitions that can not be used
without adaptions in the SESMG. However, the structure can still be copied the structure of the example_model_defintion.xlsx.
We are working on updating the US-tool to match the SESMG inputs again.
