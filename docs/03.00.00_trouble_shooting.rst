...
Error M-034: ValueError (same label)
-----------------------
**Error Message:** ::

   ValueError: cannot insert {column}, already exists

**Possible Error Cause:** The same label is used twice. Can also occur if one of the coponents is not activated.

**Debugging:** Use of unique labels, even for components that are not activated.


Error M-035: No module named 'program_files.GUI_st'
-----------------------
**Error Message:** ::

   import program_files.GUI_st.GUI_st_global_functions as GUI_functions
   ModuleNotFoundError: No module named 'program_files.GUI_st'

**Possible Error Cause:** You installed the SESMG in the Downloads directory or somewhere else with limited rights to access files interally.

**Debugging:** Move the whole SESMG installation to another directory.


Error M-036: Unsupported operand (timeseries simplification)
-----------------------
**Error Message:** ::

    preprocessing\data_preparation.py", line 39, in extract_single_periods
        for i in range(0, int(len(cluster_df) / timesteps)):
    TypeError: unsupported operand type(s) for /: 'int' and 'NoneType'

**Possible Error Cause:** Possibly you didn't choose a period at preprocessing timeseries simplification (hours, days or weeks).

**Debugging:** Make sure to fill out what is required: `Timeseries simplification <https://spreadsheet-energy-system-model-generator.readthedocs.io/en/latest/02.02.00_application.html#timeseries-simplificaton>`_ .
