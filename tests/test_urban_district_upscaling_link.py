            ...
            left=pandas.DataFrame.from_dict(
                {
                    ...
                    "bus1": ["", "central_electricity_bus", "test_cluster_pv_bus"],
                    "bus2": [
                        "",
...
def test_create_central_electricity_bus_connection(test_clustered_electricity_links):
    """ """
    sheets = {
    ...
    sheets = Link.create_central_electricity_bus_connection(
        cluster="test_cluster", sheets=sheets, standard_parameters=standard_parameters
    )

    sheets["links"] = sheets["links"].sort_index(axis=0)
    test_clustered_electricity_links["links"] = test_clustered_electricity_links[
        "links"
    ].sort_index(axis=0)
    ...
    return {
        "links": pandas.merge(
            left=pandas.DataFrame.from_dict(
                {
                    "label": [
                        "test_cluster_pv_central_electricity_link",
                        "test_cluster_pv_electricity_link",
                    ],
                    "bus1": ["test_cluster_pv_bus"] * 2,
                    "bus2": ["central_electricity_bus", "test_cluster_electricity_bus"],
                    "link_type": [
                        "building_pv_central_link",
...
def test_create_cluster_pv_links(test_cluster_pv_links_entries):
    ...
    test_cluster_pv_links_entries["links"].set_index("label", inplace=True, drop=False)
...
def test_add_cluster_naturalgas_bus_links(test_cluster_natural_gas_bus_links_entry):
    """ """
