...
def test_decentral_pv_source_entry():
            ...
            left=pandas.DataFrame.from_dict(
                {
                    ...
                    "input": [0],
                    "Temperature Inlet": [0],
                    "max. investment capacity": source["Capacity per Area (kW/m2)"]
                    * 100,
                }
            ),
            right=sources,
    ...
    # above
    sheets["sources"] = sheets["sources"].rename(
        columns={"max. investment capacity_x": "max. investment capacity"}
    )
...
def test_competition_constraint_entry():
    """ """
    pv_source = sources.loc[sources["source_type"] == "fixed photovoltaic source"]
    st_source = sources.loc[sources["source_type"] == "solar_thermal_collector"]

    return {
        "competition constraints": pandas.DataFrame.from_dict(
            {
...
def test_create_sources(
    ...
    test_sheets = pandas.concat(
        [
            test_decentral_pv_source_entry["sources"],
    ...
    # assert rather the two dataframes are equal
    pandas.testing.assert_frame_equal(
        sheets["sources"].sort_index(axis=1), test_sheets.sort_index(axis=1)
    )

    pandas.testing.assert_frame_equal(
        sheets["competition constraints"].sort_index(axis=1),
        test_competition_constraint_entry["competition constraints"].sort_index(axis=1),
    )


def test_cluster_sources_information():
    """ """
    sheets = {"sources": pandas.DataFrame.from_dict({"label": ["test_1", "test_2"]})}
    ...
    for num, test in source.iterrows():
        source_param, sheets = Source.cluster_sources_information(
            source=test, source_param=source_param, azimuth_type="north", sheets=sheets
        )

    pandas.testing.assert_frame_equal(sheets["sources"], test_sheets_clustering)
...
def test_sources_clustering():
    ...
    sheets = {"sources": pandas.DataFrame.from_dict({"label": ["testbuilding_pv_1"]})}

    sheets["sources"].set_index("label", inplace=True, drop=False)

    sheets_clustering = {
        "sources": pandas.DataFrame.from_dict(
            {
...
def test_clustered_competition_constraint_entry():
    ...
    sheets["competition constraints"] = sheets["competition constraints"].set_index(
        None, drop=True
    )
...
def test_create_cluster_sources(
    test_clustered_pv_source_entry,
    ...
    for key in sheets.keys():
        pandas.testing.assert_frame_equal(
            sheets[key].sort_index(axis=1), test_sheets[key].sort_index(axis=1)
        )
