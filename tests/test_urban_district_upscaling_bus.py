...
def test_create_standard_parameter_bus(test_elec_bus_entry, test_heat_bus_entry):
    """
...
def test_create_cluster_electricity_buses(cluster_electricity_bus_entry):
    """ """
    from program_files.urban_district_upscaling.components.Bus import (
        create_cluster_electricity_buses,
    )
    ...
    for key in sheets.keys():
        cluster_electricity_bus_entry[key].set_index("label", inplace=True, drop=False)
        pandas.testing.assert_frame_equal(
            sheets[key].sort_index(axis=1),
            cluster_electricity_bus_entry[key].sort_index(axis=1),
        )
        pandas.testing.assert_frame_equal(
            sheets[key].sort_index(axis=1), no_changes_sheets[key].sort_index(axis=1)
        )
