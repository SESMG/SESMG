...
def test_GUI_main_dict():
    ...
    return {
        "input_timeseries_algorithm": "slicing A",
...
def test_create_timeseries_parameter_list(test_GUI_main_dict):
    ...
    from program_files.GUI_st.GUI_st_global_functions import (
        create_timeseries_parameter_list,
    )
...
def test_import_GUI_input_values_json(test_GUI_main_dict):
    ...
    path_to_test_json = os.path.join(
        os.path.dirname(__file__), "test_GUI_st_cache.json"
    )

    imported_dict = import_GUI_input_values_json(json_file_path=path_to_test_json)
...
def test_create_simplification_index(test_GUI_main_dict):
    ...
    """
    from program_files.GUI_st.GUI_st_global_functions import create_simplification_index
    ...
    input_timeseries_period_dict = {"None": 0, "hours": 1, "days": 2, "weeks": 3}
    ...
    create_simplification_index(
        input_list=simpification_index_list, input_output_dict=reduced_test_dict
    )
...
def test_positive_read_markdown_document():
    ...
    """
    from program_files.GUI_st.GUI_st_global_functions import read_markdown_document
...
def test_negative_read_markdown_document():
    ...
    # creating the reduced README file as a str in a list
    reduced_readme = read_markdown_document(
        document_path=os.path.join(os.path.dirname(__file__), "README.md"),
        folder_path="empty",
    )
