...
def test_bus_entry():
    ...
    test_energy_system.add(
        Sink(
            label="test_bus_excess",
            inputs={bus: Flow(variable_costs=-0.35, emission_factor=-27)},
        )
    )
    ...
    test_energy_system.add(
        Source(
            ...
            outputs={bus: Flow(variable_costs=0.5, emission_factor=300)},
        )
    )
