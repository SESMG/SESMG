...
import pandas
import os
from program_files.urban_district_upscaling.components import Central_components
...
def test_create_central_heat_component():
    from program_files.urban_district_upscaling.components.Central_components import (
    ...
    return {
            ...
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0, 0],
                    "label": [
                        "central_test_electrolysis_transformer",
                    ...
                    "output2": ["None", "None", "central_test_heat_bus"],
                    "area": [float(0)] * 3,
        ...
        "storages": pandas.merge(
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0],
                    "label": ["central_h2_storage", "central_naturalgas_storage"],
                    "storage_type": [
                        "central_h2_storage",
                        "central_naturalgas_storage",
                    ],
                    "bus": ["central_h2_bus", "central_naturalgas_bus"],
                }
            ),
            right=storages,
            on="storage_type",
        ).drop(columns=["storage_type"]),
    }
...
def test_create_power_to_gas_system(test_create_power_to_gas_entry):
    ...
    sheets = Central_components.create_power_to_gas_system(
        ...
        sheets={
            "buses": pandas.DataFrame(columns=["label"]),
    ...
    return {
        "buses": pandas.merge(
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0],
                    "label": ["central_test_bus", "central_test_elec_bus"],
                    "bus_type": [
            ...
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0],
                    "label": [
                        "central_test_elec_central_link",
                    ...
                    "bus1": ["central_test_elec_bus", "central_naturalgas_bus"],
                    "bus2": ["central_electricity_bus", "central_test_bus"],
                }
            ),
            right=links,
            on="link_type",
...
def test_create_central_chp(test_central_CHP_entry):
    ...
    for key in sheets.keys():
        if len(sheets[key]) > 1:
            test_central_CHP_entry[key].set_index(None, inplace=True, drop=True)
        pandas.testing.assert_frame_equal(
            sheets[key].sort_index(axis=1),
