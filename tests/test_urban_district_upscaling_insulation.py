...
def test_insulation_entries():
    ...
    insulation = insulations.loc[insulations["year of construction"] == "<1918"]
        ...
        "insulation": pandas.DataFrame.from_dict(
            {
                ...
                "U-value new": [
                    float(
                        insulations.loc[
                            insulations["year of construction"] == "potential"
                        ]["window"]
                    ),
                    float(
                        insulations.loc[
                    ...
                    float(
                        insulations.loc[
                            insulations["year of construction"] == "potential flat"
                        ]["roof"]
                    ),
                ],
                "area": 3 * ["10"],
                "periodical costs": [
                    float(
                        insulations.loc[
                            insulations["year of construction"] == "periodical costs"
                        ]["window"]
                    ),
                    float(
                        insulations.loc[
                ...
                "periodical constraint costs": [
                    ...
                    float(
                        insulations.loc[
                            insulations["year of construction"]
                            == "periodical constraint costs flat"
                        ]["roof"]
                    ),
                ],
                "existing": 3 * [0],
...
def test_create_building_insulation(test_insulation_entries):
    ...
    sheets = Insulation.create_building_insulation(
        ...
        sheets=sheets,
        standard_parameters=pandas.ExcelFile(
            os.path.dirname(__file__) + r"/standard_parameters.xlsx"
        ),
    )
