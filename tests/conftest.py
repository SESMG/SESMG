...
def test_storage_decentralized_battery_entry():
    ...
    storage = storages.loc[storages["storage_type"] == "building_battery_storage"]
...
def compare_flow_attributes(flows, flows_test):
    ...
    if len(list(flows.keys())) > 0:
        ...
        if hasattr(flow_test_method, "emission_factor"):
            assert flow_test_method.emission_factor == flow_compare.emission_factor
            assert (
                flow_test_method.variable_costs.default
        ...
        if hasattr(flow_test_method, "investment"):
            ...
            if flow_compare.investment:
                assert invest_test_method.ep_costs == invest_compare.ep_costs
                assert (
                    invest_test_method.periodical_constraint_costs
                    == invest_compare.periodical_constraint_costs
                )
                assert invest_test_method.minimum == invest_compare.minimum
                assert invest_test_method.maximum == invest_compare.maximum
                assert invest_test_method.existing == invest_compare.existing
                assert invest_test_method.nonconvex == invest_compare.nonconvex
                assert invest_test_method.offset == invest_compare.offset
                assert (
                    invest_test_method.fix_constraint_costs
