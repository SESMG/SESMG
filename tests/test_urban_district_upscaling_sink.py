...
def test_create_electricity_sink(
    test_SFB_electricity_sink_entry,
...
def test_create_heat_sink(
    test_SFB_heat_sink_entry, test_COM_heat_sink_entry, test_certificate_heat_sink_entry
):
    """
    To test the creation process of electricity sinks four different
    sinks are created:
...
def test_electric_vehicle_entry():
    ...
    sheets = {
        "sinks": pandas.merge(
            ...
            right=sinks,
            on="sink_type",
        ).drop(columns=["sink_type", "nominal value_y"])
    }

    sheets["sinks"] = sheets["sinks"].rename(
        columns={"nominal value_x": "nominal value"}
    )
...
def test_create_sink_ev(test_electric_vehicle_entry):
    """ """
    time_series = pandas.ExcelFile(
        os.path.dirname(__file__) + "/ev_timeseries.xlsx"
    ).parse("ev_timeseries")

    sheets = Sink.create_sink_ev(
        building=pandas.Series(
            data={"label": "test", "distance of electric vehicles": 10000}
        ),
    ...
    assert list(time_series.loc[:, "electric_vehicle.fix"] * 10000) == list(
        sheets["time series"].loc[:, "test_electric_vehicle.fix"]
    )
...
def test_create_sinks(
    ...
    test_sheets = pandas.concat(
        [
            test_SFB_electricity_sink_entry["sinks"],
    ...
    pandas.testing.assert_frame_equal(
        sheets["sinks"].sort_index(axis=1), test_sheets.sort_index(axis=1)
    )
        ...
        "sinks": pandas.DataFrame.from_dict(
            {
                ...
                "input": [
                    "test_cluster_res_electricity_bus",
...
def test_create_cluster_electricity_sinks(test_cluster_electricity_sinks_entries):
    """ """
    sink_parameters = [
        3000,
        3000,
