...
def test_storage_clustering():
    ...
    sheets = {
        "storages": pandas.DataFrame.from_dict({"label": ["testbuilding_battery_1"]})
    }
    ...
    pandas.testing.assert_frame_equal(sheets["storages"], test_sheets_clustering)
...
def test_cluster_storage_information():
    ...
    sheets = {"storages": pandas.DataFrame.from_dict({"label": ["test_1", "test_2"]})}
    sheets["storages"].set_index("label", inplace=True, drop=False)
...
def test_clustered_battery_storage():
    ...
    sheets["storages"] = sheets["storages"].rename(
        columns={
