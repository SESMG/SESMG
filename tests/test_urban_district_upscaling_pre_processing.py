...
def test_append_component():
    """ """
    from program_files.urban_district_upscaling.pre_processing import append_component

    sheets = append_component(
        sheets={"test": pandas.DataFrame()}, sheet="test", comp_parameter={"test": 3}
    )
...
def test_read_standard_parameters():
    ...
    standard_parameters_file = pandas.ExcelFile(
        os.path.dirname(__file__) + "/standard_parameters.xlsx"
    )
    ...
    test_standard_parameters = (
        buses.loc[buses["bus_type"] == "building_heat_bus"]
        .set_index("bus_type")
        .squeeze()
    )
    test_standard_keys = test_standard_parameters.keys()

    pandas.testing.assert_series_equal(standard_parameters, test_standard_parameters)

    assert list(standard_keys) == list(test_standard_keys)


def test_create_standard_parameter_comp(test_storage_decentralized_battery_entry):
    """ """
    from program_files.urban_district_upscaling.pre_processing import (
    ...
    pandas.testing.assert_frame_equal(
        sheets["storages"], test_storage_decentralized_battery_entry["storages"]
    )
...
def test_hp_buses_entry():
    ...
    sheets = {
        "buses": pandas.merge(
            ...
            right=buses,
            on="bus_type",
        ).drop(columns=["bus_type"]),
        "links": pandas.merge(
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0, 0],
                    "label": [
                        "test_building_hp_elec_link",
...
def test_create_heat_pump_buses_links(test_hp_buses_entry):
    ...
    for key in sheets.keys():
        pandas.testing.assert_frame_equal(
            sheets[key].sort_index(axis=1), test_hp_buses_entry[key].sort_index(axis=1)
        )
    ...
    sheets = {
        "buses": pandas.merge(
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0, 0],
                    "label": ["test_electricity_bus", "test_heat_bus", "test_pv_bus"],
                    "bus_type": [
            ...
            left=pandas.DataFrame.from_dict(
                {
                    ...
                    "bus1": ["central_electricity_bus", "test_pv_bus", "test_pv_bus"],
                    "bus2": [
                        "test_electricity_bus",
                        "test_electricity_bus",
...
def test_get_central_comp_active_status():
    ...
    central = pandas.DataFrame.from_dict(
        {"technology": ["test", "test2"], "active": ["yes", "no"]}
    )

    assert get_central_comp_active_status(central=central, technology="test")
    assert not get_central_comp_active_status(central=central, technology="test2")
    ...
    sheets = {
        "buses": pandas.merge(
            left=pandas.DataFrame.from_dict(
                {
                    None: [0, 0],
                    "label": ["st_parcel_hp_elec_bus", "st_parcel_heat_bus"],
                    "bus_type": ["building_hp_electricity_bus", "building_heat_bus"],
...
def test_create_gchp(test_create_gchp_entry):
    """ """
    from program_files.urban_district_upscaling.pre_processing import create_gchp
    ...
    parcels = pandas.DataFrame.from_dict(
        {"ID parcel": ["test_parcel"], "gchp area (m²)": "100"}
    )
    ...
    gchps, sheets = create_gchp(
        tool=tool,
        parcels=parcels,
        sheets={"buses": pandas.DataFrame(), "transformers": pandas.DataFrame()},
        standard_parameters=pandas.ExcelFile(
            os.path.dirname(__file__) + "/standard_parameters.xlsx"
        ),
    )
