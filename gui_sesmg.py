...
def getFolderPath():
    ''' opens a file dilaog and sets the selected path for the variable "scenario_path"
    '''
...
def getFolderPath2():
    ''' opens a file dilaog and sets the selected path for the variable "sf_residential"
    '''
...
def getFolderPath3():
    ''' opens a file dilaog and sets the selected path for the variable "sf_non_residential"
    '''
    path = filedialog.askopenfilename(filetypes=(("Spreadsheet Files", "*.shp"), ("all files", "*.*")))
    print(path)
...
def execute_sesmg():
    #subprocess.call('Spreadsheet_Energy_System_Model_Generator.py', shell=True)
    if scenario_path.get() != "No scenario selected.":
        # comments[2].configure(text='Optimization is beeing executed ...')
        ...
        #    subprocess.call("python3 Interactive_Results.py",shell=True)
        #    subprocess.call("open http://127.0.0.1:8050/",shell=True)
        show_results()
        #subprocess.call("open Interactive_Results.py", timeout=10, shell=True)
    else:
...
def show_results():
    ...
    # Starts the new Plotlydashserver for MACOS
        ...
        subprocess.call("open http://127.0.0.1:8050/", shell=True)
    #proc = subprocess.call("open cd \..\Interactive_Results.py", timeout=10, shell=True)
    #proc()
...
# Definition des Programmfensters
window = Tk()
...
#scenario_path = os.path.join(os.path.dirname(__file__), 'scenario.xlsx')
...
sf_non_residential = StringVar(window, str(os.path.join(os.path.dirname(__file__), 'shapefiles/Nicht_Wohngebaeude.shp')))
...
file_paths = []
create_main_frame_elements(elements=selection_elements, sheet=main_frame, first_row=1, file_paths=file_paths, frame=main_frame)
