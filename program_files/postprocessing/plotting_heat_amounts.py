...
def create_heat_amount_plots(dataframes: dict, nodes_data, result_path,
                             sink_known):
    """
    ...
    for key in dataframes:
        ...
        heat_amounts_dict = st_heat_amount(components_df,
                                           "solar_thermal_flat_plate",
                                           dataframe, heat_amounts_dict)
        # heat pump dataframe
        ...
        for num, comp in df_hp.iterrows():
            if comp["heat source"] == "Ground":
                heat_amounts_dict["GCHP"].append(get_value(
                    comp["label"], "output 1/kWh", dataframe))
            elif comp["heat source"] == "Air":
                # ashp heat output
                heat_amounts_dict["ASHP"].append(get_value(
                    comp["label"], "output 1/kWh", dataframe))
        # sink dataframe
        ...
        for num, sink in df_sinks.iterrows():
            if sink_known[sink["label"]][1]:
                heat_amounts_dict["Heat_Demand"].append(get_value(
                    sink["label"], "input 1/kWh", dataframe))
        ...
        for num, storage in df_storage.iterrows():
            ...
            if "heat" in storage["bus"] and "central" not in storage["bus"]:
                value = get_value(storage["label"], "output 1/kWh",
                                  dataframe)
                heat_amounts_dict["Thermalstorage_output"].append(value)
                input_val = get_value(storage["label"], "input 1/kWh",
                                      dataframe)
                heat_amounts_dict["Thermalstorage_losses"].append(
                        input_val - value)
        ...
        for num, transformer in df_gen_transformer.iterrows():
            ...
            # (Electric Heating)
            if ("heat" in transformer["output"]
                    or "heat" in transformer["output2"]) \
                    and "central" not in transformer["input"] \
                    and "elec" in transformer["input"]:
                heat_amounts_dict["Electric_heating"].append(get_value(
                    transformer["label"], "output 1/kWh", dataframe))
            elif ("heat" in transformer["output"]
                    or "heat" in transformer["output2"]) \
                    and "central" not in transformer["input"] \
                    and "gas" in transformer["input"]:
                heat_amounts_dict["Gasheating"].append(get_value(
                    transformer["label"], "output 1/kWh", dataframe))
        ...
        for num, insulation in df_insulation.iterrows():
            ...
            cap_insulation = get_value(insulation["label"], "capacity/kW",
                                       dataframe)
            value_sink = get_value(insulation["sink"], "input 1/kWh",
                                   dataframe)
    ...
    for plot in plot_dict:
        plot.stackplot(heat_amounts.reductionco2,
