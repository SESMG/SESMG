...
label_dict = {
    "_electricity_demand": ["electricity demand", "input 1/kWh"],
    "_parcel_gchp_elec_link": ["electricity demand heat pump (GCHP)", "output 1/kWh"],
    ...
    "_oilheating_transformer": ["heat supply (oil heating system)", "output 1/kWh"],
    "_electricheating_transformer": [
        "heat supply (electric heating system)",
        "output 1/kWh",
    ],
    "_wall-insulation": ["heat savings (wall)", "output 1/kWh"],
    "_roof-insulation": ["heat savings (roof)", "output 1/kWh"],
    "_window-insulation": ["heat savings (window)", "output 1/kWh"],
    "_1_solarthermal_source": ["heat supply (solar thermal collector)", "output 1/kWh"],
    "_ashp_transformer": ["heat supply (ashp)", "output 1/kWh"],
}


def create_building_specific_results(
    us_sheet_raw_data: str,
    ...
    components_raw_data.drop(
        components_raw_data.loc[components_raw_data["capacity/kW"] == 0].index,
        inplace=True,
    )
    ...
    for building in list_of_buildings:
        ...
        for comp in label_dict:
            if str(building) + comp in components_raw_data["ID"].tolist():
                ...
                overview[building][label_dict.get(comp)[0]] = components_raw_data_row[
                    label_dict.get(comp)[1]
                ].values[0]
