...
def return_component_value(componentid, table):
    ...
    return {"x": componentdates, "y": values, "mode": "lines", "name": str(name)}
...
demo_app.layout = html.Div(
    children=[
        ...
        html.H1(
            children="Spreadsheet Energy System Model Generator -" "Interactive Results"
        ),
        ...
        html.Div(
            children=[
                html.Div(
                    "capacity/kW - means the ouput capacity of "
        ...
        dcc.Dropdown(
            ...
            # can be selected
            options=[{"label": i, "value": i} for i in list_of_components],
        ...
        dcc.Graph(
            ...
            figure={
                "data": [{"x": dates, "y": [0, 0], "mode": "lines"}],
                # Defines the layout of the graph
                "layout": {
                    "title": "Component Time Series",
                    "xaxis": {"title": "date"},
                    "yaxis": {"title": "performance [kW] / storage capacity [kWh]"},
                },
            },
        ),
    ]
...
def update_graph(component_ids):
    if component_ids is None:
        ...
        fig = {
            "data": [return_component_value(ID, df_table) for ID in component_ids],
            "layout": {
                "title": "Component Time Series",
                "xaxis": {"title": "date"},
                "yaxis": {"title": "performance [kW] / " "storage capacity [kWh]"},
