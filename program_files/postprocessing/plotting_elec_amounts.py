...
def pv_electricity_amount(
    ...
    for num, comp in df_pv_or_st.iterrows():
        ...
        amounts_dict = add_value_to_amounts_dict(
            label="PV", value_am=value_am, amounts_dict=amounts_dict
        )

        amounts_dict = get_pv_st_dir(
            c_dict=amounts_dict, value=value_am, comp_type="PV", comp=comp
        )
        # calculate the pv excess
        value = get_value(
            label=str(comp["output"]) + "_excess",
            column="input 1/kWh",
            dataframe=dataframe,
        )
        amounts_dict = add_value_to_amounts_dict(
            label="PV_excess", value_am=value, amounts_dict=amounts_dict
        )
    return amounts_dict


def get_electric_timeseries_sources(
    ...
    for num, comp in df_source.iterrows():
        if comp["sector"] == "electricity":
            value_am = get_value(comp["label"], "output 1/kWh", dataframe)
            amounts_dict = add_value_to_amounts_dict(
                label="Timeseries", value_am=value_am, amounts_dict=amounts_dict
            )

            # collect all output buses of the energy system's pv system to
            # calculate the pv excess
            value_am = get_value(comp["output"] + "_excess", "input 1/kWh", dataframe)
            amounts_dict = add_value_to_amounts_dict(
                label="Timeseries_excess", value_am=value_am, amounts_dict=amounts_dict
            )
    return amounts_dict


def get_electric_windpower_sources(
    components_df: pandas.DataFrame, dataframe: pandas.DataFrame, amounts_dict: dict
) -> dict:
    """
    ...
    for num, comp in df_source.iterrows():
        value_am = get_value(comp["label"], "output 1/kWh", dataframe)
        amounts_dict = add_value_to_amounts_dict(
            label="Windpower", value_am=value_am, amounts_dict=amounts_dict
        )
        ...
        value_am = get_value(comp["output"] + "_excess", "input 1/kWh", dataframe)
        amounts_dict = add_value_to_amounts_dict(
            label="Windpower_excess", value_am=value_am, amounts_dict=amounts_dict
        )
    return amounts_dict


def get_st_electricity_amounts(
    components_df: pandas.DataFrame, amounts_dict: dict, dataframe: pandas.DataFrame
) -> dict:
    """
    ...
    for num, comp in df_st.iterrows():
        ...
        amounts_dict = add_value_to_amounts_dict(
            label="ST_electricity", value_am=value, amounts_dict=amounts_dict
        )

    return amounts_dict


def sink_electricity_amounts(
    components_df: pandas.DataFrame,
    sink_known: dict,
    dataframe: pandas.DataFrame,
    amounts_dict: dict,
    ...
    for num, sink in df_sinks.iterrows():
        if sink_known[sink["label"]][0]:
            ...
            amounts_dict = add_value_to_amounts_dict(
                label="Electricity Demand", value_am=value, amounts_dict=amounts_dict
            )
    return amounts_dict


def generic_transformer_electricity_amounts(
    components_df: pandas.DataFrame, dataframe: pandas.DataFrame, amounts_dict: dict
) -> dict:
    """
    ...
    for num, transformer in df_gen_transformer.iterrows():
        ...
        if transformer["sector"] == "electricity" and transformer["output2"] == "None":
            amounts_dict = add_value_to_amounts_dict(
        ...
        elif "heat" in transformer["sector"] and transformer["output2"] != "None":
...
def get_heat_pump_electricity_amounts(
    components_df: pandas.DataFrame, dataframe: pandas.DataFrame, amounts_dict: dict
) -> dict:
    """
...
def battery_storage_electricity_amounts(
    components_df: pandas.DataFrame, dataframe: pandas.DataFrame, amounts_dict: dict
) -> dict:
    """
...
def get_grid_import(
    ...
    for num, comp in df_buses.iterrows():
        if "electricity" in comp["sector"]:
            value = get_value(comp["label"] + "_shortage", "output 1/kWh", dataframe)
            amounts_dict = add_value_to_amounts_dict(
                label="grid_import", value_am=value, amounts_dict=amounts_dict
            )
    return amounts_dict
...
def collect_electricity_amounts(
    ...
    for key in dataframes:
        ...
        elec_amounts_dict = get_grid_import(
            components_df=components_df,
            dataframe=dataframe,
            amounts_dict=elec_amounts_dict,
        )
