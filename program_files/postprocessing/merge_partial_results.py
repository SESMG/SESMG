...
def run_pareto(
    limits, files, gui_variables, timeseries_prep_param, directory, result_folders
):
    for limit in limits:
...
def get_constraints(result_paths):
    ...
    for folder in result_paths["1"]:
        ...
        result["constraints/CU"] = result["constraints/CU"].astype(float)
        constraints.append(sum(result["constraints/CU"]))
    return constraints


def create_transformation_scenarios(constraints, scenario_names, result_path, limits):
    counter = 0
    files = {}
    path = os.path.join(
    ...
    for scenario in scenario_names:
        ...
        for limit in limits:
            ...
            writer = pd.ExcelWriter(
                path + "/" + scenario_names + str(limit) + ".xlsx", engine="xlsxwriter"
            )
