    ...
    for i, b in nodes_data["buses"].iterrows():
        ...
        if b["active"]:
            ...
            if b["excess"]:
                ...
                nodes.append(solph.Sink(label=b["label"] + "_excess", inputs=inputs))
            ...
            if b["shortage"]:
                ...
                nodes.append(
                    solph.Source(label=b["label"] + "_shortage", outputs=outputs)
                )
...
class Sources:
    ...
    def pv_source(self, so: dict, my_weather_pandas_dataframe):
        ...
        my_weather_pandas_dataframe["ghi"] = (
            my_weather_pandas_dataframe.dirhi + my_weather_pandas_dataframe.dhi
        )
    ...
    def windpower_source(self, so: dict, weather_df_wind):
        ...
        weather_df_wind = weather_df_wind[
            ["windspeed", "temperature", "z0", "pressure"]
        ]
    ...
    def solar_heat_source(self, so, data):
        ...
        self.nodes_sources.append(
            solph.Transformer(
                ...
                inputs={
                    self.busd[so["label"] + "_bus"]: solph.Flow(variable_costs=0),
        ...
        logging.info(
            ...
            + "Source created: "
            + so["label"]
            + ", Max Heat power output per year and m²: {:2.2f}".format(
                numpy.sum(collectors_heat)
            )
            + " kWh/(m²a)"
    ...
    def __init__(
        self, nodes_data: dict, nodes: list, busd: dict, time_series, weather_data
    ):
        """
...
class Sinks:
    ...
    def create_sink(self, de: dict, timeseries_args: dict):
        ...
        self.nodes_sinks.append(
            solph.Sink(
                label=de["label"],
                inputs={self.busd[de["input"]]: solph.Flow(**timeseries_args)},
            )
        )
    ...
    def timeseries_sink(self, de, nodes_data):
        ...
        if de["fixed"] == 0:
            ...
            args.update(
                {
                    "min": nodes_data[de["label"] + ".min"].tolist(),
    ...
    def slp_sink(self, de: dict, nodes_data: dict, weather_data):
        ...
        # creates time series
        if (
            de["load profile"] in heat_slps_commercial
    ...
    def __init__(
        self, nodes_data: dict, busd: dict, nodes: list, time_series, weather_data
    ):
...
class Transformers:
    ...
    def compression_heat_transformer(self, tf: dict, data):
        ...
        self.nodes_transformer.append(
            solph.Source(
                label=cmpr_heat_transformer_label,
                outputs={
                    self.busd[tf["label"] + temp + "_bus"]: solph.Flow(
                        investment=solph.Investment(
                            ep_costs=0, minimum=0, maximum=maximum, existing=0
                        ),
                        variable_costs=0,
                    )
                },
        ...
        inputs = {
            "inputs": {
                ...
                self.busd[tf["label"] + temp + "_bus"]: solph.Flow(variable_costs=0),
            }
        }
        outputs = {
            "outputs": {
    ...
    def absorption_heat_transformer(self, tf, data):
        ...
        ddt = abs_hp_chiller.calc_characteristic_temp(
            ...
            coef_a=char_para[(char_para["name"] == chiller_name)]["a"].values[0],
            coef_e=char_para[(char_para["name"] == chiller_name)]["e"].values[0],
            method="kuehn_and_ziegler",
        )
        # Calculation of cooling capacity
        q_dots_evap = abs_hp_chiller.calc_heat_flux(
            ddts=ddt,
            coef_s=char_para[(char_para["name"] == chiller_name)]["s_E"].values[0],
            coef_r=char_para[(char_para["name"] == chiller_name)]["r_E"].values[0],
            method="kuehn_and_ziegler",
        )
        # Calculation of driving heat
        q_dots_gen = abs_hp_chiller.calc_heat_flux(
            ddts=ddt,
            coef_s=char_para[(char_para["name"] == chiller_name)]["s_G"].values[0],
            coef_r=char_para[(char_para["name"] == chiller_name)]["r_G"].values[0],
            method="kuehn_and_ziegler",
        )
        # Calculation of COPs
...
class Storages:
    ...
    def stratified_thermal_storage(self, s, data):
        ...
        if not (s["min. investment capacity"] > 0 or s["existing capacity"] > 0):
                    ...
                    solph.custom.Link(
                        ...
                        outputs={
                            self.busd[link["bus2"]]: solph.Flow(
                                ...
                                emission_factor=link["variable constraint costs"],
                                investment=solph.Investment(
                                    ep_costs=ep_costs,
                                    ...
                                    offset=link["fix investment costs"],
                                ),
                            ),
                            self.busd[link["bus1"]]: solph.Flow(
                                variable_costs=link["variable output costs"],
