...
def main_result_summary():
    ...
    cost1.metric(
        ...
        value=round(df_summary["Total System Costs"], 1),
        delta="1.2 °F",
    )
    cost2.metric(
        label="Total Constraint Costs",
        value=round(df_summary["Total Constraint Costs"], 1),
        delta="1.2 °F",
    )
    cost3.metric(
        label="Total Variable Costs",
        value=round(df_summary["Total Variable Costs"], 1),
        delta="-1.2 °F",
    )
    cost4.metric(
        label="Total Periodical Costs",
        value=round(df_summary["Total Periodical Costs"], 1),
        delta="1.2 °F",
    )
    ...
    ener1.metric(
        ...
        value=round(df_summary["Total Energy Demand"], 1),
        delta="1.2 °F",
    )
    ener2.metric(
        label="Total Energy Usage",
        value=round(df_summary["Total Energy Usage"], 1),
        delta="1.2 °F",
    )
...
app_mode = st.sidebar.selectbox(
    "Choose the app mode",
    ["Main SESMG Application", "Urban District Upscaling Tool", "SESMG Demo Tool"],
)
