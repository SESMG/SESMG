...
def restructuring_links(sheets_clustering, building, cluster, sink_parameters,
    ...
    for i, j in sheets_clustering["links"][
        ...
        if str(building[0]) in j["bus1"] and \
                "central_electricity" in j["bus2"] and "pv_bus" in j["bus1"]:
            sheets["links"] = sheets["links"].drop(index=j["label"])
        ...
        if str(building[0]) in j["bus1"] and str(building[0]) in j["bus2"] and\
                "pv_bus" in j["bus1"]:
            sheets["links"] = sheets["links"].drop(index=j["label"])
        ...
        if str(building[0]) in j["bus2"] and "electricity_bus" in j["bus2"] \
