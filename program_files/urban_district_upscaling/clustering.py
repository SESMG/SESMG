...
def clustering_transformers(sheets, sink_parameters, transformer_parameters,
            ...
            if transformer_parameters[i][0] > 0:
                ...
                if i == "gasheating":
                    sheets = Bus.create_cluster_averaged_bus(
                        ...
                        and str(cluster) + "_gchp_building_link" \
...
def create_cluster_heat_bus(transformer_parameters, clustering_dh,
                            sink_parameters, cluster, sheets):
    # create cluster heat bus if it consists rather the
    # opportunity for an investment in electricheating,
    # gasheating, ashp or gchp
    if transformer_parameters["electricheating"][0] > 0 \
...
def collect_clustering_information(building, heat_buses_gchps, sheets,
                                   source_param, storage_parameters,
                                   transformer_parameters):

    # collect cluster intern source information
    source_param, sheets = Source.sources_clustering(
        source_param=source_param, building=building, sheets=sheets,
    ...
    heat_buses_gchps, transformer_parameters, sheets = \
        Transformer.transformer_clustering(
            building=building, sheets_clustering=sheets_clustering,
            transf_param=transformer_parameters, sheets=sheets,
    ...
    storage_parameters, sheets = \
        Storage.storage_clustering(
            building=building, sheets_clustering=sheets_clustering,
            storage_parameter=storage_parameters, sheets=sheets)
...
def collect_builing_information(cluster_ids, cluster,
    ...
    for building in cluster_ids:
        ...
        source_param, heat_buses_gchps, transformer_parameters, \
            storage_parameters, sheets = collect_clustering_information(
                building, heat_buses_gchps, sheets, source_param,
...
def create_cluster_components(standard_parameters, sink_parameters, cluster,
    ...
    sheets = clustering_transformers(
            sheets, sink_parameters, transformer_parameters, cluster,
            standard_parameters)
    # SOURCES
    ...
    sheets = create_cluster_heat_bus(
        transformer_parameters=transformer_parameters, cluster=cluster,
        clustering_dh=clustering_dh, sink_parameters=sink_parameters,
        sheets=sheets)
