...
def create_decentral_overview(components_csv_data):
    ...
    decentral_components_list = [
        ...
        "_2_solarthermal_source_collector",
        "_3_solarthermal_source_collector",
        "_4_solarthermal_source_collector",
        "_5_solarthermal_source_collector",
    ]
    ...
    for i in decentral_components_from_csv:
        ...
        for roofnum in range(5):
            ...
            maximum_pv = (
                components_csv_data.loc[
                    components_csv_data["ID"].str.contains(
                        str(i) + "_" + str(roofnum + 1) + "_pv_source"
                    )
                ]["max. invest./kW"]
            ).values
            maximum_st = (
                components_csv_data.loc[
                    components_csv_data["ID"].str.contains(
                        ...
                        + "_"
                        + str(roofnum + 1)
                        + "_solarthermal_source_collector"
                    )
                ]["max. invest./kW"]
            ).values
            ...
            installed_pv_roof = (
                components_csv_data.loc[
                    components_csv_data["ID"].str.contains(
                        str(i) + decentral_components_list[roofnum]
                    )
                ]["investment/kW"]
            ).values
            installed_pv_roof = (
            ...
            installed_st_roof = (
                components_csv_data.loc[
                    components_csv_data["ID"].str.contains(
                        str(i) + decentral_components_list[-(5 - roofnum)]
                    )
                ]["investment/kW"]
            ).values
            installed_st_roof = (
        ...
        decentral_components_dict = {
            "Building": str(i),
            "PV 1": installed_power[0],
            "Max. PV 1": maximums_pv[0],
            "PV 2": installed_power[1],
            "Max. PV 2": maximums_pv[1],
            "PV 3": installed_power[2],
            "Max. PV 3": maximums_pv[2],
            "PV 4": installed_power[3],
            "Max. PV 4": maximums_pv[3],
            "PV 5": installed_power[4],
            "Max. PV 5": maximums_pv[4],
            "Installed PV": installed_pv,
            "Max. PV": max_total_pv,
            "STC 1": installed_power[11],
            "Max. STC 1": maximums_st[0],
            "STC 2": installed_power[12],
            "Max. STC 2": maximums_st[1],
            "STC 3": installed_power[13],
            "Max. STC 3": maximums_st[2],
            "STC 4": installed_power[14],
            "Max. STC 4": maximums_st[3],
            "STC 5": installed_power[15],
            "Max. STC 5": maximums_st[4],
            "Installed STC": installed_st,
            "Max. STC": max_total_st,
            "Gasheating-System": installed_power[5],
            "ASHP": installed_power[6],
            "GCHP": installed_power[7],
            "Battery-Storage": installed_power[8],
...
def create_central_overview(components_csv_data):
    ...
    for comp in central_components:
        ...
        variable_central = (
            components_csv_data.loc[components_csv_data["ID"].str.contains(comp)][
                "investment/kW"
            ]
        ).values
        variable_central = (
