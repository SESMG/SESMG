...
def execute_sesmg():
	...
	"""
	runs = pd.read_csv(os.path.dirname(os.path.dirname(os.path.dirname(os.path.join(__file__)))) + "/Geplante_Modelllaeufe_20220303.csv")
	print(runs)
	for num, run in runs.iterrows():
		...
		timeseries_prep_param = [run["algorithm"],
									run["index"],
									run["criterion"],
									run["period"],
