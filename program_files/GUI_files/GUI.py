...
from program_files.GUI_files.urban_district_upscaling_GUI import UpscalingFrameClass
...
class GUI(MethodsGUI):
    ...
    def execute_sesmg(self):
        ...
        if self.gui_variables["scenario_path"].get():
            ...
            pre_model_path = (
                str(self.gui_variables["save_path"].get()) + "/pre_model_results"
            )
            ...
            if len(limits) > 1 or limits[0] != "":
                merge_partial_results.run_pareto(
                    limits,
                    self.gui_variables["scenario_path"].get(),
                    self.gui_variables,
                    timeseries_prep_param,
                    pre_modeling,
                ...
                for scenario in self.gui_variables["scenario_path"].get().split("\n"):
                    ...
                    self.gui_variables["save_path"].set(
                        str(
                            os.path.join(
                                self.gui_variables["save_path_directory"].get()
                            )
                            + "/"
                            + scenario_name
    ...
    def __init__(self):
        ...
        super().__init__(
            "SESMG - Spreadsheet Energy System Model Generator", "0.x", "1200x1050"
        )
        global tab_control
        ...
        row = 4
        self.create_heading(
            self.frames[0], "Timeseries Preparation", 0, row, "w", rowspan=2
        )
        column = 3
        ...
        row += 1
        row = (
            self.create_cb_lines(
                self.frames[0], checkboxes_parameters, row, self.gui_variables
            )
            + 1
        )
        ...
        # result checkboxes
        self.create_heading(
            self.frames[0], "Result processing parameters", 0, row, "w", True
        )
        checkboxes_parameters = {
        ...
        execution_elements = {
            ...
            "Select DH calculations folder": [
                lambda: self.get_path("folder", self.gui_variables["dh_path"]),
                "Change",
                "dh_path",
            ],
        ...
        row = (
            self.create_button_lines(
                self.frames[0], execution_elements, row, self.gui_variables
            )
            + 1
        )
        ...
        self.create_button_lines(
            self.frames[0], analyzing_elements, row, self.gui_variables
        )
