...
class MethodsGUI(tk.Tk):
    ...
    @staticmethod
    def create_heading(
        frame, text, column, row, sticky, bold=False, columnspan=None, rowspan=None
    ):
        """
        ...
        if bold:
            ...
            font = "Helvetica 10"
        if type(text) == str:
            Label(frame, text=text, font=font).grid(
                column=column,
                row=row,
                sticky=sticky,
                rowspan=rowspan,
            ...
            Label(frame, textvariable=text, font=font).grid(
    ...
    def create_button_lines(self, frame, elements, row, gui_variables):
        ...
        for element in elements:
            ...
            if elements[element][2] != "":
                self.create_heading(
                    frame,
                    gui_variables[elements[element][2]],
                    2,
                    row,
                    "w",
                    columnspan=6,
    ...
    def create_cb_lines(self, frame, elements, row, gui_variables):
        ...
        for param in elements:
            ...
            self.create_checkbox(frame, gui_variables[elements[param]], 1, row)
        return row
