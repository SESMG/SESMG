...
def sesmg_main(
    ...
    if sys.platform.startswith("win"):
        ...
        scheme_path = os.path.join(
            os.path.dirname(__file__) + r"/technical_data/hierarchical_selection"
            r"_schemes.xlsx"
        )
    ...
    t3 = Thread(
        target=create_objects.Transformers, args=(nodes_data, nodes, busd, weather_data)
    )
    t3.start()
    # CREATES STORAGE OBJECTS AS DEFINED IN THE SCENARIO FILE AND ADDS THEM TO
    # THE lIST OF COMPONENTS
    t4 = Thread(
        target=create_objects.Storages,
        args=(
            nodes_data,
            nodes,
            busd,
    ...
    create_results.Results(
        nodes_data, om, esys, result_path, console_log=console_results
    )
