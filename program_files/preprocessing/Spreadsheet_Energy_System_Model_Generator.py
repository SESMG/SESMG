...
def sesmg_main(
    ...
    if criterion_switch:
        pareto_optimization.change_optimization_criterion(nodes_data=nodes_data)
    ...
    thread3 = Thread(target=Transformer.Transformers, args=(nodes_data, nodes, busd))
    thread3.start()
    ...
    if (
        len(
            nodes_data["buses"][
                ~nodes_data["buses"]["district heating conn."].isin(["0", 0])
            ]
        )
        > 0
    ):
...
def sesmg_main_including_premodel(
    ...
    solver: str,
    cluster_dh,
    pre_model_timeseries_prep: list,
    ...
    logging.info(
        "Pre-modeling results will be stored in: " + result_path + str("/pre_model")
    )
