...
from program_files.preprocessing.data_preparation import (
    ...
    extract_single_periods,
)


def create_new_random_data_set(
    ...
    for index in range(len(column_names)):
        ...
        data_set_column = extract_single_periods(
            data_set=data_set, column_name=column_names[index], period=period
        )

        reference_data_set = []
        for r_index in range(len(random_integers)):
            column = r_index if weatherdata_or_timeseries else random_integers[r_index]
...
def random_sampling(nodes_data: dict, period: str, number_of_samples: int) -> None:
    """
    ...
    prep_weather_data = create_new_random_data_set(
        random_integers=random_integers, data_set=data_set, period=period
    )
    ...
    try:
        adaption_clusters = int(len(prep_weather_data) / period_dict.get(period))
    except TypeError:
        raise ValueError("Non supported period")

    # adapt costs and energy system date range
    variable_costs_date_adaption(
        nodes_data=nodes_data, clusters=int(adaption_clusters), period=period
    )
