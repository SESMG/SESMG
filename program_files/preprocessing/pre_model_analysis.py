...
def filter_result_component_types(
    components: pandas.DataFrame, component_type: str
) -> pandas.DataFrame:
    """
...
def update_component_investment_decisions(
    ...
    model_definition_type_name: str,
    ...
    result_components = filter_result_component_types(
        components=components, component_type=result_type_name
    )

    # read the component type model definition sheet
    components_xlsx = pandas.read_excel(
        io=model_definition_path, sheet_name=model_definition_type_name
    )
    ...
    try:
        ...
        component_type_switch_dict.get(model_definition_type_name)(
            components_xlsx=components_xlsx, result_components=result_components
        )
...
def technical_pre_selection(
    components_xlsx: pandas.DataFrame, result_components: pandas.DataFrame
) -> list:
    """
    ...
    for num, component in components_xlsx.iterrows():
        ...
        if label in result_components.index.values:
            ...
            if (
                str(result_components.loc[label]["investment/kW"]) == "0.0"
                and float(result_components.loc[label]["max. invest./kW"]) > 0
            ):
...
def tightening_investment_boundaries(
    components_xlsx: pandas.DataFrame,
    ...
    for num, component in components_xlsx.iterrows():
        ...
        if (
            label in result_components.index.values
            and float(result_components.loc[label]["max. invest./kW"]) > 0
        ):
...
def deactivate_respective_competition_constraints(
    model_definition_path: str, list_of_deactivated_components: list
) -> pandas.DataFrame:
    """
...
def dh_technical_pre_selection(
    components_xlsx: pandas.DataFrame, result_components: pandas.DataFrame
) -> None:
    """
    ...
    logging.info(
        "\t WARNING: IF THE ORIGINAL SECTION NAME CONTAINED THE "
...
def bus_technical_pre_selection(
    components_xlsx: pandas.DataFrame, result_components: pandas.DataFrame
) -> None:
    """
    ...
    logging.info(
        "WARNING: IF THE ORIGINAL BUS NAME CONTAINED THE STRING "
    ...
    logging.info(
        "WARNING: IF THE ORIGINAL BUS NAME ARE DUPLICATES BEFORE "
    ...
    for num, dh_bus in bus_xlsx.iterrows():
        ...
        if str(dh_bus["district heating conn."]) not in no_invest_list:
            if (
                label not in dh_investment_list
...
def insulation_technical_pre_selection(
    components_xlsx: pandas.DataFrame, result_components: pandas.DataFrame
) -> None:
    """
        deactivates district heating investment decisions for which no
...
def update_model_according_pre_model_results(
    ...
    model_definition_xlsx = pandas.read_excel(io=model_definition_path, sheet_name=None)
    ...
    for sub_list in component_types:
        ...
        # deactivated components
        complete_list_of_deactivated_components += list_of_deactivated_components
    ...
    logging.info(
        "\t Model definition updated according to the results of the " "pre-model."
    )
