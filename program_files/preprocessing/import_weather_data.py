...
def import_open_fred_windpowerlib(nodes_data, lat, lon):
    ...
    windpowerlib_df = open_FRED_weather_windpowerlib_single_location.df(
        location=location, lib="windpowerlib"
    )
    ...
    for num, row in windpowerlib_df.iterrows():
        if num % 2 == 1:
            ...
            windpowerlib_df.loc[num, "temperature"] = (
                float(windpowerlib_df.loc[num, "temperature"]) - 273.15
            )
...
def import_open_fred_pvlib(nodes_data, lat, lon):
    ...
    for num, row in pvlib_df.iterrows():
        if num != 0:
            ...
            pvlib_df.loc[num, "index"] = pvlib_df.loc[num, "index"] - timedelta(
    ...
    nodes_data["weather data"]["dirhi"] = (
        pvlib_df["ghi"].copy() - pvlib_df["dhi"].copy()
    )
