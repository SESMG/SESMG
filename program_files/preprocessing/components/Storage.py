...
class Storages:
            ...
            GenericStorage(
                ...
                inputs={
                    self.busd[storage["bus"]]: Flow(
                        ...
                        emission_factor=storage["variable input constraint costs"],
                    )
                },
                outputs={
                    self.busd[storage["bus"]]: Flow(
                        variable_costs=storage["variable output costs"],
                        emission_factor=storage["variable output constraint costs"],
                    )
                },
                ...
                invest_relation_output_capacity=storage["output/capacity ratio"],
                investment=Investment(
                    ep_costs=storage["periodical costs"],
                    periodical_constraint_costs=storage["periodical constraint costs"],
                    existing=storage["existing capacity"],
                    minimum=storage["min. investment capacity"],
                    maximum=storage["max. investment capacity"],
                    nonconvex=True if storage["non-convex investment"] == 1 else False,
                    offset=storage["fix investment costs"],
                    fix_constraint_costs=storage["fix investment constraint costs"],
                ),
            )
        )
    ...
    def __init__(self, nodes_data: dict, nodes: list, busd: dict):
        ...
        # active storages
        storages = nodes_data["storages"][nodes_data["storages"]["active"] == 1]
