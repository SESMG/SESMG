...
class Transformers:
    ...
    def genericchp_transformer(self, tf: dict, nd: dict):
        ...
        self.nodes_transformer.append(GenericCHP(
            label=tf['label'],
            fuel_input={self.busd[tf['input']]: Flow(
                H_L_FG_share_max=[
                ...
                variable_costs=tf['variable input costs'],
                emission_factor=tf['variable input constraint costs'])},
            electrical_output={self.busd[tf['output']]: Flow(
                investment=Investment(
                    ep_costs=tf['periodical costs'],
                    periodical_constraint_costs=tf[
                        'periodical constraint costs'],
                ...
                P_max_woDH=[tf['max. electric power without district heating']
                            for p in range(0, periods)],
                P_min_woDH=[tf['min. electric power without district heating']
                            for p in range(0, periods)],
                Eta_el_max_woDH=[tf['el. eff. at max. fuel flow w/o distr. '
                                    'heating'] for p in range(0, periods)],
                Eta_el_min_woDH=[tf['el. eff. at min. fuel flow w/o distr. '
                                    'heating'] for p in range(0, periods)],
                variable_costs=tf['variable output costs'],
                emission_factor=tf['variable output constraint costs'])},
            heat_output={self.busd[tf['output2']]: Flow(
                Q_CW_min=[tf['minimal therm. condenser load to cooling water']
                          for p in range(0, periods)],
                variable_costs=tf['variable output costs 2'],
                emission_factor=tf['variable output constraint costs 2'])},
