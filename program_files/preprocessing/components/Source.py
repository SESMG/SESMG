...
class Sources:
    ...
    def timeseries_source(self, so: dict):
        ...
        if so["fixed"] == 1:
            ...
            args = {
                "min": self.timeseries[so["label"] + ".min"].tolist(),
    ...
    def create_feedin_source(
        ...
        if output is None:
            # starts the create_source method with the parameters set before
            self.create_source(so, args, self.busd[so["output"]], variable_costs)
        else:
            # starts the create_source method with the parameters set before
            self.create_source(so, args, output, variable_costs)
        # returns logging info
    ...
    def solar_heat_source(self, so: dict):
        ...
        self.create_feedin_source(collectors_heat, so, output, [0, 0])
        ...
        logging.info(
            ...
            "{:2.2f}".format(numpy.sum(collectors_heat / so["Conversion Factor"]))
