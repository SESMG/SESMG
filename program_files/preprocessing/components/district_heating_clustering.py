...
def connect_clustered_dh_to_system(
    oemof_opti_model, busd: dict, thermal_network: ThermalNetwork, nodes_data: dict
    ...
    oemof_opti_model = district_heating_calculations.calc_heat_pipe_attributes(
        oemof_opti_model=oemof_opti_model, pipe_types=nodes_data["pipe types"]
    )
    ...
    for num, consumer in thermal_network.components["consumers"].iterrows():
        ...
        pipe = pipes.loc[pipes["to_node"] == "consumers-{}".format(consumer["id"])]
        ...
        inputs = {
            oemof_opti_model.buses[
                ...
                investment=solph.Investment(
                    ep_costs=float(link["capex_pipes"] * pipe["length"][0]),
                    periodical_constraint_costs=float(
                        link["periodical_constraint_costs"] * pipe["length"][0]
                    ),
        ...
        oemof_opti_model.nodes.append(
            solph.Transformer(
                label=(
                    "pipe-clustered-{}-".format(consumer["id"]) + str(pipe["length"][0])
                ),
                inputs=inputs,
                outputs={
                    busd["clustered_consumers_{}".format(consumer["id"])]: solph.Flow(
                        emission_factor=0, variable_costs=0
                    )
                },
                conversion_factors={
...
def get_forks_street_section_wise(nodes_data: dict, forks: pandas.DataFrame) -> dict:
    """
...
def get_pipe_lengths_street_section_wise(
    forks_street: dict, thermal_network: ThermalNetwork, pipes: pandas.DataFrame
) -> (ThermalNetwork, dict):
    """
    ...
    for street in forks_street:
        if forks_street[street]:
            ...
            for num, pipe in pipes.iterrows():
                ...
                # consumers
                if (
                    pipe["from_node"] in forks_street[street]
                    ...
                    thermal_network.components["pipes"] = thermal_network.components[
                        "pipes"
                    ].drop(index=pipe["id"])
    return thermal_network, streets_pipe_length


def get_street_sections_consumers_information(
    streets_pipe_length: dict,
...
def clear_thermal_network_dataframes(thermal_network: ThermalNetwork) -> ThermalNetwork:
    """
    ...
    for num, pipe in thermal_network.components["pipes"].iterrows():
        thermal_network.components["pipes"] = thermal_network.components["pipes"].drop(
            index=num
        )
    thermal_network.components["pipes"] = thermal_network.components[
        "pipes"
    ...
    for num, fork in thermal_network.components["forks"].iterrows():
        thermal_network.components["forks"] = thermal_network.components["forks"].drop(
            index=num
        )
    thermal_network.components["forks"] = thermal_network.components[
        "forks"
    ].reset_index(drop=True)
...
def clustering_dh_network(
    nodes_data: dict, thermal_network: ThermalNetwork
) -> ThermalNetwork:
    """
    ...
    forks_street = get_forks_street_section_wise(nodes_data=nodes_data, forks=forks)

    thermal_network, streets_pipe_length = get_pipe_lengths_street_section_wise(
        forks_street=forks_street, thermal_network=thermal_network, pipes=pipes
    )

    thermal_network, streets_consumer = get_street_sections_consumers_information(
        streets_pipe_length=streets_pipe_length,
        thermal_network=thermal_network,
        consumers=consumers,
    )

    thermal_network = clear_thermal_network_dataframes(thermal_network=thermal_network)
    ...
    for street in streets_consumer:
        if streets_consumer[street][0] > 0:
                ...
                [
                    pandas.Series(
                        data={
                            ...
                            "lat": float(
                                streets_consumer[street][1]
                                / streets_consumer[street][0]
                            ),
                            "lon": float(
                                streets_consumer[street][2]
                                / streets_consumer[street][0]
                            ),
            ...
            foot_point = district_heating_calculations.get_nearest_perp_foot_point(
                {
                    "lat": float(
                        streets_consumer[street][1] / streets_consumer[street][0]
                    ),
                    "lon": float(
                        streets_consumer[street][2] / streets_consumer[street][0]
                    ),
                },
                street_sections,
                counter,
            ...
            thermal_network.components["pipes"] = pandas.concat(
                [thermal_network.components["pipes"], new_pipe_part]
            )
    ...
    thermal_network = district_heating.create_intersection_forks(
        street_sec=nodes_data["district heating"], thermal_net=thermal_network
    )
    ...
    thermal_network = district_heating.create_supply_line(
        streets=nodes_data["district heating"], thermal_net=thermal_network
    )

    return district_heating.adapt_dhnx_style(
        thermal_net=thermal_network, cluster_dh=False
    )
