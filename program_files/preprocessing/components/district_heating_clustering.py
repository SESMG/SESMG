def connect_clustered_dh_to_system(oemof_opti_model, busd):
    ...
    for num, consumer in thermal_network.components["consumers"].iterrows():
        ...
        oemof_opti_model.nodes.append(
            solph.Transformer(
                label=(
                    "pipe-clustered{}-".format(consumer["id"]) + str(consumer["length"])
                ),
                inputs={
                    oemof_opti_model.buses[
                        ...
                        investment=solph.Investment(
                            ep_costs=float(
                                component_param.loc["clustered_consumer_link"]["costs"]
                                * consumer["length"]
                            ),
                            ...
                            periodical_constraint_costs=float(
                                component_param.loc["clustered_consumer_link"][
                                    "constraint costs"
                                ]
                                * consumer["length"]
                            ),
...
def clustering_dh_network(nodes_data):
                ...
                if pipe["from_node"] in forks_street[street]:
                    if "consumers" in pipe["to_node"]:
                        ...
                        thermal_network.components[
                            "pipes"
                        ] = thermal_network.components["pipes"].drop(index=pipe["id"])
    streets_consumer = {}
    ...
    for street in streets_consumer:
        ...
        thermal_network.components["consumers"] = pd.concat(
            [
        ...
        foot_point = get_nearest_perp_foot_point(
            {
                "lat": float(streets_consumer[street][1] / streets_consumer[street][0]),
                "lon": float(streets_consumer[street][2] / streets_consumer[street][0]),
            },
            street_sections,
            counter,
            "consumers",
        ...
        thermal_network.components["pipes"] = pd.concat(
            [
